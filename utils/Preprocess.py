...
def pattern_to_feature(ds, patterns, encoding='utf-8', header=0, index_col=0):
    ...
    return [list(filter(lambda column: re.match(pattern, column), ds.columns))
                for pattern in (patterns if type(patterns) == list else [patterns])]
...
def clean_special_feature(ds, features, method='delete_feature', save_path=None, encoding='utf-8', header=0, index_col=0):
    '''
    # Params:
